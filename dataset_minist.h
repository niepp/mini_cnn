#ifndef __DATASET_MINIST_H__
#define __DATASET_MINIST_H__
...
unsigned char* read_file(std::string file_path)
{
	...
	std::ifstream fsread;
	fsread.open(file_path, std::fstream::in | std::fstream::binary);
	if (!fsread)
	{
...
void read_dataset(std::string data_dir, varray_vec &img_vec, varray_vec &lab_vec, varray_vec &test_img_vec, index_vec &test_lab_vec)
{
	// read train data
	// train images
	unsigned char *images = read_file(data_dir + "train-images.idx3-ubyte");
	int index = 0;
	...
	// train labels
	unsigned char *labels = read_file(data_dir + "train-labels.idx1-ubyte");
	int idx = 0;
	...
	// read test data
	// test images
	unsigned char *test_images = read_file(data_dir + "t10k-images.idx3-ubyte");
	int test_idx = 0;
	...
	// test labels
	unsigned char *test_labels = read_file(data_dir + "t10k-labels.idx1-ubyte");
	int lab_idx = 0;
